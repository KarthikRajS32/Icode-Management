...
/**
 * Premium Teacher Portal Layout Wrapper
 */
export const TeacherLayout = () => {
  ...
  // Email notifications drawer state
  const [showNotifications, setShowNotifications] = useState(false);

  // Find detailed teacher info if available
          ...
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            {!isCollapsed && (
              <div className="flex flex-col">
                <span className="font-extrabold text-slate-800 dark:text-slate-100 text-sm tracking-tight">ICode Teacher</span>
                <span className="text-[10px] text-indigo-500 font-semibold tracking-wider uppercase">Portal</span>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-20 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800/80 px-6 flex items-center justify-between gap-4 z-20 shadow-xs relative">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center">
              <Badge variant="indigo" className="font-bold text-[10px]">
                {teacherDetails?.subject || 'ICode Faculty'}
              </Badge>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <Dropdown
              trigger={
                <button className="flex items-center gap-2.5 hover:opacity-90 transition-opacity">
                  <Avatar name={currentUser?.name || 'Teacher'} size="sm" showStatus status="online" />
                  <div className="hidden lg:flex flex-col text-left">
                    <span className="text-xs font-bold leading-tight">{currentUser?.name || 'Faculty Member'}</span>
            ...
            >
              <div className="px-4 py-2 text-xs border-b border-slate-100 dark:border-slate-800">
                ...
                <p className="text-slate-400 mt-0.5">{teacherDetails?.subject || 'Teacher'}</p>
              </div>
              <DropdownItem onClick={() => navigate('/teacher')}>
                <LayoutDashboard size={14} /> My Dashboard
              </DropdownItem>
              <DropdownItem onClick={() => navigate('/teacher/classrooms')}>
                <School size={14} /> My Classrooms
              </DropdownItem>
              <DropdownItem onClick={() => navigate('/teacher/activities')}>
                <ClipboardList size={14} /> Student Activities
              </DropdownItem>
          ...
          <aside className="w-96 max-w-full bg-white dark:bg-slate-900 border-l border-slate-100 dark:border-slate-800 h-full flex flex-col relative z-50 shadow-2xl animate-in slide-in-from-right duration-250">
            ...
            <div className="p-6 border-b border-slate-100 dark:border-slate-800/80 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-extrabold text-slate-800 dark:text-slate-100 text-sm">Dispatched Activity Emails</h3>
              </div>
              <button
            ...
            <div className="flex-grow overflow-y-auto p-6 flex flex-col gap-4">
              {mailReceipts.length === 0 ? (
                <div className="flex flex-col items-center justify-center text-center gap-2 h-full text-slate-400">
                  ...
                  <div>
                    <p className="font-bold text-xs">No notifications sent yet</p>
                    <p className="text-[10px] text-slate-400 mt-1 max-w-[200px]">
                      Simulated activity email logs will be displayed here in real time.
                    </p>
                  </div>
                </div>
              ) : (
